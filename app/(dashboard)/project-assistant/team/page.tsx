...
export default function PATeamPage() {
  ...
  useEffect(() => {
    ...
    const loadTeam = async () => {
      setLoading(true)
      try {
        // The API scopes this to the caller's own department for PAs.
        const [usersResponse, departmentResponse] = await Promise.all([
          fetch("/api/users"),
        ...
        if (!usersResponse.ok) {
          throw new Error(usersData?.error || "Unable to load your department members")
        }

        setMembers(usersData.users || [])
        if (departmentResponse.ok && departmentData?.department) {
          setDepartmentName(departmentData.department.name)
        }
      } catch (loadError) {
        if (active) {
  ...
  const rows = useMemo(
    ...
    [members, currentUserId, filterRole, filterMonth, filterYear]
  )
  ...
  return (
    <div className="space-y-8 pb-12">
      <PageHeader
        ...
        description={
          departmentName
            ? `Everyone in ${departmentName} — the faculty who can book you, and the assistants you work alongside.`
            : "Everyone in your department — the faculty who can book you, and the assistants you work alongside."
        }
      />
        ...
        <div className="flex flex-col gap-4 border-b border-slate-100 p-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-base font-semibold text-slate-900">
              <Users className="h-4 w-4 text-indigo-600" /> Department members
            </h2>
          ...
          <UserFilter
            ...
            exportFilename={`team-${departmentName || "department"}`}
          />
        </div>
