 ...
 * these for every department, edited from the Global Settings page.
 */
...
export function ScheduleSettingsForm({ onSaved }: { onSaved?: (settings: ScheduleSettings) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/settings/schedule", {
        ...
        // geofenceEnabled is owned by the Geo-fence tab; leaving it out keeps
        // whatever was saved there.
        body: JSON.stringify({
          ...form,
          geofenceEnabled: undefined,
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col">
      ...
      <div className="flex justify-end border-t border-slate-100 bg-slate-50/60 px-5 py-4">
        <Button type="submit" className="cursor-pointer" disabled={submitting || loading || !form}>
